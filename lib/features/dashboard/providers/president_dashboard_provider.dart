...
final totalCollectionsProvider =
...
final monthlyCollectionsProvider =
  ...
  return paymentsAsync.whenData((list) {
    ...
    final approved =
        list.where((p) => p.status == PaymentStatus.approved).toList();
    ...
    for (int i = 11; i >= 0; i--) {
      ...
      final amount = approved
          .where((p) =>
              p.createdAt.year == keyYear && p.createdAt.month == keyMonth)
          .fold<double>(0.0, (sum, p) => sum + p.amount);
...
final collectionsByMethodProvider =
  ...
  return paymentsAsync.whenData((list) {
    ...
    for (final payment in list.where((p) => p.status == PaymentStatus.approved)) {
      result[payment.method] = (result[payment.method] ?? 0.0) + payment.amount;
    }
    return result;
  });
});
    ...
    () {
      ...
      return levies.map((levy) {
        ...
        final target = levyObligations.fold<double>(
            0.0, (sum, o) => sum + o.amount);
        final collected = levyObligations.fold<double>(
            0.0, (sum, o) => sum + o.paidAmount);
        final percentage =
...
final financialOverviewProvider =
  ...
  return _combine<Map<String, double>>(
    ...
    () {
      ...
      final totalExpenses =
          allExpenses.fold<double>(0.0, (sum, e) => sum + e.amount);
      return {
...
final presidentDashboardProvider =
  ...
  return _combine<PresidentDashboardData>(
    ...
    () {
      final financial = financialAsync.valueOrNull ??
          {
