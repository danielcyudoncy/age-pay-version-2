...
final memberPaymentsStreamProvider = StreamProvider.autoDispose.family<List<PaymentModel>, String>((ref, memberId) {
...
final memberTotalOutstandingProvider = Provider.autoDispose.family<AsyncValue<double>, String>((ref, memberId) {
  ...
  return asyncObligations.whenData((list) => list.fold(0.0, (sum, o) => sum + o.outstandingBalance));
});

final memberActiveLeviesProvider = Provider.autoDispose.family<AsyncValue<int>, String>((ref, memberId) {
  final asyncObligations = ref.watch(memberActiveObligationsProvider(memberId));
  return asyncObligations.whenData((list) => list.length);
});

final memberRecentPaymentsProvider = Provider.autoDispose.family<AsyncValue<List<PaymentModel>>, String>((ref, memberId) {
  final asyncPayments = ref.watch(memberPaymentsStreamProvider(memberId));
...
final memberRegistrationFeeStatusProvider = Provider.autoDispose.family<AsyncValue<bool>, String>((ref, memberId) {
  final asyncObligations = ref.watch(memberObligationsProvider(memberId));
  return asyncObligations.whenData((list) =>
      list.any((o) => o.type == ObligationType.registrationFee && o.status == ObligationStatus.paid));
});
