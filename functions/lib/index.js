...
exports.verifyPayment = exports.submitBankTransfer = exports.recordCashPayment = exports.verifyPaystackTransaction = exports.initializePaystackPayment = exports.createLevyWithObligations = exports.createObligationsOnLevy = void 0;
 ...
 */
exports.createObligationsOnLevy = functions.firestore
    ...
    await batch.commit();
    console.log(`Created ${membersSnap.size} obligations for levy ${levyId}`);
});
/**
 * Callable function: create a levy and immediately generate obligations
 * for all active members atomically via batch.
 */
exports.createLevyWithObligations = functions.https.onCall(async (data, context) => {
    ...
    if (!['treasurer', 'president', 'superAdmin'].includes(role)) {
        throw new functions.https.HttpsError('permission-denied', 'Only treasurers, presidents, or admins can create levies');
    }
    const { title, description, type, amountPerMember, dueDate, targetGroup, } = data;
    if (!title || !amountPerMember || !dueDate) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: title, amountPerMember, dueDate');
    }
    ...
    console.log(`Created levy ${levyRef.id} with ${obligationsCreated} obligations`);
 ...
 */
exports.initializePaystackPayment = functions
    ...
    if (!email || !amountNaira || !reference) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: email, amountNaira, reference');
    }
...
exports.verifyPaystackTransaction = functions
    ...
    if (!reference || !memberId || !obligationIds || !amountPaid) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: reference, memberId, obligationIds, amountPaid');
    }
    const secretKey = process.env.PAYSTACK_SECRET_KEY || '';
    if (!secretKey) {
        throw new functions.https.HttpsError('failed-precondition', 'Paystack secret key not configured');
    ...
    try {
        ...
        if (!result.status || result.data.status !== 'success') {
            throw new Error(result.data?.gateway_response || 'Transaction verification failed');
        }
        const verifiedAmount = result.data.amount / 100; // kobo to naira
        const timestamp = admin.firestore.FieldValue.serverTimestamp();
        const now = admin.firestore.Timestamp.now();
        // Build allocations
        const allocations = obligationIds.map((id) => ({
            obligationId: id,
            amount: Number(verifiedAmount) / obligationIds.length, // simple split for now
        }));
        ...
        for (const alloc of allocations) {
            ...
            const currentPaid = obligationData['paidAmount'] || 0;
            const totalAmount = obligationData['amount'] || 0;
        ...
        throw new functions.https.HttpsError('internal', error.message || 'Transaction verification failed');
    }
});
/**
...
exports.recordCashPayment = functions.https.onCall(async (data, context) => {
    ...
    const role = userDoc.data()['role'];
    if (!['treasurer', 'president', 'superAdmin'].includes(role)) {
        throw new functions.https.HttpsError('permission-denied', 'Only treasurers, presidents, or admins can record cash payments');
    }
    const { memberId, obligationIds, amount, notes, recordedBy } = data;
    if (!memberId || !obligationIds || !amount || !recordedBy) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: memberId, obligationIds, amount, recordedBy');
    }
    const now = admin.firestore.Timestamp.now();
    // Build allocations
    const allocations = obligationIds.map((id) => ({
        obligationId: id,
        amount: Number(amount) / obligationIds.length,
    }));
    // Create payment document
    const paymentRef = db.collection('payments').doc();
    await paymentRef.set({
 ...
 */
exports.submitBankTransfer = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    ...
    if (!memberId || !obligationIds || !amount || !transferReference || !bankName || !receiptUrl) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: memberId, obligationIds, amount, transferReference, bankName, receiptUrl');
    }
    const now = admin.firestore.Timestamp.now();
    // Build allocations
    const allocations = obligationIds.map((id) => ({
...
exports.verifyPayment = functions.https.onCall(async (data, context) => {
    ...
    const role = userDoc.data()['role'];
    if (!['treasurer', 'president', 'superAdmin'].includes(role)) {
        throw new functions.https.HttpsError('permission-denied', 'Only treasurers, presidents, or admins can verify payments');
    }
    const { paymentId, action, verifiedBy, notes } = data;
    if (!paymentId || !action || !verifiedBy) {
        throw new functions.https.HttpsError('invalid-argument', 'Missing required fields: paymentId, action, verifiedBy');
    }
